...
def load_volume(csv_path):
    ...
    with open(csv_path) as f:
        ...
        for r in rdr: V.append(float(r["exp_vol"]))
...
def main():
    ...
    results = []
    for label, path in [("baseline", args.baseline), ("SA-QUBO", args.sa), ("QA-QUBO", args.sqa)]:
        try:
            r = load_json(path); fills = r["fills"]
            results.append((label, fills, r.get("ac_cost", ac_cost(fills, V, args.eta, args.lambda_risk, args.sigma))))
        except Exception:
            pass
    ...
    plt.subplot(1,2,1); plt.title("AC cost (lower=better)"); plt.bar(labels, ac_vals); plt.xticks(rotation=20)
    plt.subplot(1,2,2); plt.title("Alpha-like L1 to volume profile"); plt.bar(labels, alpha_vals); plt.xticks(rotation=20)
    ...
    plt.plot(list(V / V.max() * max(max(f) for _,f,_ in results)), linestyle='--', label="volume shape (scaled)")
    plt.title("Fills per slice"); plt.legend(); plt.tight_layout(); plt.savefig("schedules.png", dpi=150)
