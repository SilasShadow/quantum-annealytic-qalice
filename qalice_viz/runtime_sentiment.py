...
def build_runtime_bar(prefix: str = "with_sentiment", sampler: Literal["sa", "sqa"] = "sa") -> str:
    ...
    # Prepare data
    methods = ['KMeans', 'QUBO:Coarsen', 'QUBO:Build', 'QUBO:Anneal', 'QUBO:Decode', 'QUBO:Expand', 'QUBO:Total']
    times = [
        kmeans_time,
        timings['coarsen_sec'],
    ...
    bars = plt.bar(methods, times, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2'])
    
    plt.title(f'Runtime Comparison: {prefix.replace("_", " ").title()} ({sampler.upper()})')
